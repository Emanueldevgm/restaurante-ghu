...
export const LoginDTOSchema = z.object({
    email: z.string().min(0).transform(v => v?.trim() || undefined).pipe(
        z.union([z.literal(undefined), z.string().email('Email inválido')])
    ).optional(),
    telefone: z.string().min(0).transform(v => v?.trim() || undefined).pipe(
        z.union([z.literal(undefined), z.string().min(7, 'Telefone deve ter pelo menos 7 dígitos')])
    ).optional(),
...
export const RegisterDTOSchema = z.object({
    ...
    senha: z.string().min(6, 'Senha deve ter pelo menos 6 caracteres'),
    bi: z.string().min(0).transform(v => v?.trim() || undefined).optional(),
    nif: z.string().min(0).transform(v => v?.trim() || undefined).optional(),
...
export const AdminCreateUserDTOSchema = z.object({
    ...
    telefone: z.string().min(7, 'Telefone deve ter pelo menos 7 dígitos'),
    telefone_alternativo: z.string().min(0).transform(v => v?.trim() || undefined).optional(),
    senha: z.string().min(6, 'Senha deve ter pelo menos 6 caracteres'),
    bi: z.string().min(0).transform(v => v?.trim() || undefined).optional(),
    nif: z.string().min(0).transform(v => v?.trim() || undefined).optional(),
    role: z.enum(['cliente', 'administrador', 'garcom', 'cozinha', 'entregador', 'gerente']).default('cliente'),
...
const CreateItemCardapioBaseSchema = z.object({
    ...
    preco_kz: z.number().positive('Preço deve ser positivo').finite(),
    preco_promocional_kz: z.number().positive().finite().optional(),
    tempo_preparo: z.number().int().positive('Tempo deve ser em minutos positivos').optional(),
...
export const CreateItemCardapioDTOSchema = CreateItemCardapioBaseSchema.refine(
    ...
    { message: 'Preço promocional deve ser menor que o preço normal', path: ['preco_promocional_kz'] }
);

export const UpdateItemCardapioDTOSchema = CreateItemCardapioBaseSchema.partial();
