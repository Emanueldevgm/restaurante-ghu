...
import { LogOut, Clock, CheckCircle2, Package, ShoppingBag, Star } from 'lucide-react';
...
export function ClientDashboard() {
  ...
  // =============================================
  // 🔑 PROTECÇÃO: Se não está carregando e não tem user, redireciona
  // (redundante se usares ProtectedRoute, mas seguro como fallback)
  // =============================================
  ...
  const pedidosAtivos = orders.filter((o: Order) =>
    ['pendente', 'confirmado', 'em_preparo'].includes(o.status),
  ).length;
      ...
      <main className="container mx-auto px-4 py-8">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
              ...
              <Card className="bg-gradient-to-br from-accent/10 to-transparent border-0 shadow-md">
                <CardContent className="p-6">
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    {orders.length > 0 ? format(new Date(orders[0].created_at), 'dd/MM') : '—'}
                  </p>
                </CardContent>
              </Card>
            ...
            <Card className="border-0 shadow-lg">
              ...
              <CardContent>
                {ordersLoading ? (
                  ...
                  <p className="text-center py-6 text-muted-foreground">Nenhum pedido ainda.</p>
                ) : (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {order.tipo} · {format(new Date(order.created_at), 'dd/MM HH:mm')}
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
          ...
          <TabsContent value="reviews" className="space-y-6">
            ...
            ) : orders.filter((o: Order) => o.status === 'entregue').length === 0 ? (
              <Card className="border-dashed border-2 bg-transparent">
                <CardContent className="pt-12 pb-12 text-center">
                  <p className="text-muted-foreground">
                    Nenhum pedido completado para avaliar ainda.
                  </p>
                </CardContent>
              </Card>
            ) : (
