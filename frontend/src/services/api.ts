...
export const authApi = {
    login: async (email: string, telefone: string, senha: string): Promise<AuthResponse> => {
        const payload: any = { senha };
        if (email?.trim()) payload.email = email;
        if (telefone?.trim()) payload.telefone = telefone;
        const response = await api.post<AuthResponse>('/auth/login', payload);
        return response.data;
    },
    register: async (data: { nome_completo: string; telefone: string; email?: string; senha: string }): Promise<AuthResponse> => {
        const payload: any = { nome_completo: data.nome_completo, telefone: data.telefone, senha: data.senha };
    ...
    changePassword: async (senhaAtual: string, novaSenha: string): Promise<{ success: boolean; message: string }> => {
        const response = await api.put('/auth/change-password', { senha_atual: senhaAtual, nova_senha: novaSenha });
        return response.data;
    },
    ...
    createUser: async (userData: {
        nome_completo: string; email?: string; telefone: string; telefone_alternativo?: string;
        senha: string; bi?: string; nif?: string; role?: string; status?: string;
        data_nascimento?: string; genero?: string;
    }): Promise<{ success: boolean; message: string; data: { user: User } }> => {
        const response = await api.post('/auth/users', userData);
        return response.data;
    },
    updateUser: async (id: string, data: Partial<User>): Promise<{ success: boolean; message: string }> => {
    ...
    updateUserStatus: async (id: string, status: string): Promise<{ success: boolean; message: string }> => {
...
export const menuApi = {
    ...
    createCategory: async (data: { nome: string; descricao?: string; imagem?: string }): Promise<{ success: boolean; message: string }> => {
        const response = await api.post('/menu/categories', data);
        return response.data;
    },
    updateCategory: async (id: string, data: Partial<Category>): Promise<{ success: boolean; message: string }> => {
    ...
    getMenuItems: async (filters?: { categoria_id?: string; destaque?: boolean; prato_do_dia?: boolean; vegetariano?: boolean; search?: string; limit?: number }): Promise<MenuItem[]> => {
...
export const reservationsApi = {
    createReservation: async (data: {
        nome_cliente: string; telefone_cliente: string; email_cliente?: string;
        quantidade_pessoas: number; data_reserva: string; hora_reserva: string;
        mesa_id?: string; ocasiao_especial?: string; observacoes?: string;
    ...
    getAllReservations: async (filters?: { status?: string; data?: string; mesa_id?: string }): Promise<Reservation[]> => {
    ...
    updateReservationStatus: async (id: string, status: string): Promise<{ success: boolean; message: string }> => {
...
export const ordersApi = {
    createOrder: async (data: {
        tipo: 'delivery' | 'retirada' | 'mesa'; endereco_id?: string; mesa_id?: string;
        itens: Array<{ item_cardapio_id: string; quantidade: number; observacoes?: string }>;
        observacoes?: string; observacoes_entrega?: string; cupom_codigo?: string;
    }): Promise<{ success: boolean; message: string; data?: any }> => {
        const response = await api.post('/orders', data);
        return response.data;
    },
    getMyOrders: async (filters?: { status?: string; tipo?: string }): Promise<any[]> => {
    ...
    getAllOrders: async (filters?: { status?: string; tipo?: string; data_inicio?: string; data_fim?: string; search?: string }): Promise<any[]> => {
        const response = await api.get('/orders/admin/all', { params: filters });
        return response.data.data;
    },
    updateOrderStatus: async (id: string, status: string): Promise<{ success: boolean; message: string }> => {
...
export interface Review {
    id: string; pedido_id: string; usuario_id: string; usuario_nome?: string;
    nota: number; comentario?: string; resposta_restaurante?: string; created_at: string;
}

export const reviewsApi = {
    getReviews: async (limit = 10, offset = 0): Promise<{ data: Review[] }> => {
    ...
    getRestaurantRating: async (): Promise<any> => {
        const response = await api.get('/reviews/restaurant/rating');
        return response.data.data;
    },
    createReview: async (data: { pedido_id: string; nota: number; comentario?: string }): Promise<{ success: boolean; message: string; data?: { id: string } }> => {
